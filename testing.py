...
def Database():
    ...
    cursor.execute("SELECT * FROM `admin` WHERE `username` = 'ankit' AND `password` = 'ankit'")
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `admin` (username, password) VALUES('ankit', 'ankit')")
        conn.commit()
...
def LoginForm():
    global lbl_result
    TopLoginForm = Frame(loginform, width=600, height=100, bd=1, relief=SUNKEN)
    ...
    btn_login = Button(MidLoginForm, text="Login", font=('arial', 18), width=30, bg="#008080",command=Login)
...
def AddNewForm():
    ...
    btn_add = Button(MidAddNew, text="Save", font=('arial', 18), width=30, bg="#008080", command=AddNew)
